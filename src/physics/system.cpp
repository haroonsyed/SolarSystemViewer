...
void System::setBodiesGPU(std::vector<Body>& bodies) {

  // For now let's hard code the VRAM usage
  float vram_tree_size_GB = 0.5;

  m_SSBO_BODIES_COUNT = bodies.size();
  m_SSBO_TREE_COUNT = (unsigned int)(vram_tree_size_GB * 1e9 / sizeof(TreeCell));
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(Body) * m_SSBO_BODIES_COUNT, &bodies[0], GL_DYNAMIC_DRAW);
...
void System::updateUsingNaiveGPU(float adjustedTimeFactor) {
  ...
  unsigned int tmp = m_SSBO_BODIES;
  m_SSBO_BODIES = m_SSBO_BODIES_OUTPUT;
  m_SSBO_BODIES_OUTPUT = tmp;
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 4, m_SSBO_BODIES);
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 6, m_SSBO_BODIES_OUTPUT);
